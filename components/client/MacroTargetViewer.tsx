  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        .from('macro_targets')
        .select('protein_g, carbs_g, fat_g, notes')
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5 space-y-5" style={{
        ...
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-white text-sm">Your Daily Macro Targets</h3>
        </div>
        ...
        <div className="space-y-3">
          {STEPS.map(({ n, text }) => (
            <div key={n} className="flex gap-3">
              <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5" style={{ background: 'rgba(201,168,76,0.15)', color: '#C9A84C' }}>
                {n}
              </span>
